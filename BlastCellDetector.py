...
#   # 60530,23100
  ...
  st.text("")
  st.subheader("Who Dies From This Cancer?")

  st.write("<body style='text-align: justify;'> Death rates from leukemia are higher among the elderly. People with leukemia have many\
    treatment options, and treatment for leukemia can often control the disease and its symptoms. \
    Leukemia is the seventh leading cause of cancer death in the United States. The death rate was \
    6.3 per 100,000 men and women per year based on 2014–2018 deaths, age-adjusted.</body>", unsafe_allow_html = True)
  show = Image.open("stats\\5.PNG")
  st.image(show,use_column_width = True)
  st.subheader("Which age is at risk")

  st.write("<body style='text-align: justify;'>Number of people having leukemia peaks between the age group of 65-74 and dives maximum in the age\
  	group of 35-44.</body>", unsafe_allow_html = True)
  st.text("")
  st.text("")
  ...
  st.text("")
  st.write("<body style = 'text-align:justify;'>People in the age group of 75-84 are at maximum risk of death.\
  	</body>",unsafe_allow_html = True)
  show = Image.open("stats\\8.PNG")
  st.image(show,use_column_width = True)

  st.text("")
  st.write("<body style = 'text-align:justify;'>After studying about leukemia in detail and the working of CNN we decided to make a \
  	model toclassify leukemia from cell images.\
  	</body>",unsafe_allow_html = True)
...
def About():
  ...
  original = Image.open("UsedImages\\tensor.png")
  cols = st.beta_columns([1,2])
  cols[0].image(original,use_column_width = True)
  cols[1].markdown("<body style='text-align: justify;'>TensorFlow is an end-to-end open source platform for machine learning.\
   It has a comprehensive, flexible ecosystem of tools, libraries and community resources that lets researchers push the \
   state-of-the-art in ML and developers easily build and deploy ML powered applications.</body>",unsafe_allow_html=True)
  cols[1].markdown("<body style='text-align: justify;'><i>Version : 2.2.0 </i></body>",unsafe_allow_html=True)
